...
- If no health/medical tier exists but a health/medical plan exists, the app can infer
  listed dependents as enrolled (controlled in the sidebar).
- Only health/medical fields are used for plan/tier/election detection. Dental, vision,
  life, disability, and other ancillary fields are ignored.
...
OUTPUT_COLUMNS: list[str] = [f["name"] for f in FIELDS]
_FIELD_IDX_BY_NAME: dict[str, int] = {f["name"]: i for i, f in enumerate(FIELDS)}
_FIELD_BY_NAME: dict[str, dict] = {f["name"]: f for f in FIELDS}
...
HEADER_ALIASES: dict[str, str] = {
    ...
    "primary address state territory": "State", "primary address state": "State", "home state": "State",
    ...
    "medical election": "Health Election", "benefit election": "Health Election",
    "medical waive reason": "Health Election",
    # Health plan fields
    ...
    "medical plan name": "Current Health Plan", "health plan name": "Current Health Plan",
    ...
    "coverage level": "Current Health Plan Tier", "plan tier": "Current Health Plan Tier",
    "tier": "Current Health Plan Tier", "medical coverage tier": "Current Health Plan Tier",
    ...
    "rate": "Hourly Rate", "hourly wage": "Hourly Rate",
    ...
    **{letter.lower(): field["name"] for letter, field in zip(_COL_LETTERS, FIELDS)},
}
...
_add_rel("Employee",
         "employee", "ee", "emp", "subscriber", "self",
         "primary", "insured", "member", "worker")
_add_rel("Spouse",
         "spouse", "sp", "husband", "wife", "partner",
         "domestic partner", "life partner", "registered domestic partner",
         "common law spouse", "common law partner", "spse", "sps")
_add_rel("Child",
         "child", "ch", "dependent", "dep", "daughter", "son",
         "stepchild", "step child", "adopted child", "foster child",
         "stepson", "stepdaughter", "child of domestic partner", "child child")

_ELECTION_ENROLL: set[str] = {
    "enroll", "e", "yes", "y", "enrolled", "participating",
    "active", "covered", "elect", "elected",
}
_ELECTION_WAIVE: set[str] = {
    "waive", "w", "no", "n", "waived", "decline", "declined",
    "opt out", "optout", "not participating", "waiving",
}

_TIER_EMP_ONLY: set[str] = {
    "employee only", "employee", "ee only", "employee only coverage",
    "single", "individual", "self only",
}
_TIER_EMP_SPOUSE: set[str] = {
    "employee spouse", "employee and spouse", "employee + spouse",
    "ee spouse", "ee + spouse", "ee sp", "employee plus spouse",
}
_TIER_EMP_CHILDREN: set[str] = {
    "employee children", "employee child", "employee + children",
    "employee + child", "ee children", "ee child", "ee + children",
    "ee + child", "employee plus children", "parent child", "parent children",
}
_TIER_FAMILY: set[str] = {
    "family", "employee family", "employee + family", "ee family",
    "ee + family", "employee spouse children", "employee + spouse + children",
    "employee spouse child", "ee spouse children", "ee + spouse + children",
}
_TIER_WAIVE: set[str] = {
    "waive", "waived", "decline", "declined",
    "no coverage", "none", "not enrolled", "no election",
}

_VALID_TIERS: set[str] = {"Employee Only", "Employee + Spouse", "Employee + Children", "Family"}
...
def normalize_relationship(v: object) -> tuple[str, Optional[str]]:
    ...
    if key in _RELATIONSHIP_MAP:
        ...
        note = (
            f"Relationship normalised: '{original}' -> '{canonical}'"
            if canonical.lower() != original.lower() else None
        )
...
def _norm_tier_key(s: str) -> str:
    key = _norm(s.replace("&", " and ").replace("/", " ").replace("-", " "))
...
def normalize_tier(v: object) -> tuple[str, Optional[str]]:
    original = str(v or "").strip()
    key = _norm_tier_key(original)
    if not key:
        return "", None

    def _norm_set(options: set[str]) -> set[str]:
        return {_norm_tier_key(o) for o in options}

    compact = _compact(original)

    if key in _norm_set(_TIER_WAIVE) or any(x in compact for x in ("waive", "decline", "nocoverage")):
        canonical = "Waive"
    elif key in _norm_set(_TIER_FAMILY) or "family" in compact or (
        "spouse" in compact and ("child" in compact or "children" in compact)
    ):
        canonical = "Family"
    elif key in _norm_set(_TIER_EMP_SPOUSE) or (
        "spouse" in compact and "child" not in compact and "children" not in compact
    ):
        canonical = "Employee + Spouse"
    elif key in _norm_set(_TIER_EMP_CHILDREN) or "child" in compact or "children" in compact:
        canonical = "Employee + Children"
    elif key in _norm_set(_TIER_EMP_ONLY) or compact in {"ee", "eo", "employeeonly", "selfonly", "individual"}:
        canonical = "Employee Only"
    else:
        return original, None
...
def _pad_zip(z: object) -> str:
    v = str(z or "").strip()
    return v.zfill(5) if re.fullmatch(r"\d+", v) and 0 < len(v) < 5 else v
    ...
    for fi, field_def in enumerate(FIELDS):
        for ci, hn in enumerate(headers_norm):
            if ci in used_df_cols:
                continue
            if HEADER_ALIASES.get(hn) == field_def["name"]:
                mapping[fi] = ci
                used_df_cols.add(ci)
                break
        ...
        for fi, field_def in enumerate(FIELDS):
            ...
            if best_ci is not None and best_score >= FUZZY_THRESHOLD:
                ...
                mapping_notes.append(
                    f"Fuzzy mapped '{raw_headers[best_ci]}' -> '{field_def['name']}' (score {best_score})"
                )
...
def _looks_health_header(header: str) -> bool:
    h = _norm(header)
    return (
        not _is_ancillary_header(h)
        and (any(term in h for term in HEALTH_TERMS) or "medical" in h or "med " in f"{h} ")
    )
...
def _pick_col(df: pd.DataFrame, aliases: list[str], *, health_only: bool = False) -> Optional[str]:
    ...
    if FUZZY_AVAILABLE:
        ...
        for a in alias_norm:
            match = rf_process.extractOne(a, norm_headers, scorer=fuzz.token_sort_ratio)
            if match:
...
def build_horizontal_column_map(df: pd.DataFrame) -> HorizontalColumnMap:
    p = lambda aliases, **kw: _pick_col(df, aliases, **kw)
    h = True  # health_only shorthand
    cm = HorizontalColumnMap()
    cm.employee_first    = p(["First Name", "Employee First Name", "EE First Name", "First", "Given Name"])
    cm.employee_last     = p(["Last Name", "Employee Last Name", "EE Last Name", "Last", "Surname", "Family Name"])
    cm.employee_id       = p(["Employee ID", "Employee Number", "Employee #", "Worker ID", "Person Number"])
    cm.employee_dob      = p(["BIRTH DATE", "DOB", "Employee DOB", "Employee Date of Birth", "Date of Birth", "Birthdate"])
    cm.employee_gender   = p(["Gender", "Sex"])
    cm.employee_email    = p(["Email", "Email Address", "Work Email", "Personal Email"])
    cm.address1          = p(["PRIMARY ADDRESS LINE 1", "Address Line 1", "Street Address", "Home Address Line 1"])
    cm.address2          = p(["PRIMARY ADDRESS LINE 2", "Address Line 2", "Home Address Line 2"])
    cm.city              = p(["PRIMARY ADDRESS - CITY", "Primary Address City", "City", "Home City"])
    cm.state             = p(["PRIMARY ADDRESS - STATE / TERRITORY", "Primary Address State", "State", "Home State"])
    cm.zip_code          = p(["PRIMARY ADDRESS - ZIP CODE", "Primary Address Zip Code", "Zip Code", "Zip", "Home Zip"])
    cm.county            = p(["County", "Home County", "Primary Address County"])
    cm.worksite_zip      = p(["WORKSITE ZIP CODE", "Primary Worksite Zip Code", "Worksite Zip", "Work Zip", "Work Location Zip"])
    cm.worksite_county   = p(["Primary Worksite County", "Worksite County", "Work County", "Work Location County"])
    cm.ichra_class       = p(["ICHRA Class", "Benefit Class", "Employee Class", "Class"])
    cm.worker_category   = p(["WORKER CATEGORY", "Worker Category", "Employee Type", "Employment Type"])
    cm.position_status   = p(["POSITION STATUS", "Employment Status", "Status"])
    cm.health_election   = p(["Health Election", "Medical Election", "Medical Coverage Election", "Coverage Election"], health_only=h)
    cm.health_waive_reason = p(["MEDICAL WAIVE REASON", "Health Waive Reason", "Waive Reason"], health_only=h)
    cm.plan_vendor       = p(["Current Health Plan Vendor", "Health Carrier", "Medical Carrier", "Carrier", "Medical Vendor"], health_only=h)
    cm.plan_name         = p(["MEDICAL PLAN NAME", "Medical Plan Name", "Health Plan Name", "Current Health Plan", "Plan Name"], health_only=h)
    cm.plan_tier         = p(["Current Health Plan Tier", "Medical Plan Tier", "Medical Coverage Tier", "Coverage Tier", "Coverage Level", "Benefit Tier", "Election Tier"], health_only=h)
    cm.oop_single        = p(["Current Health Plan OOP (single)", "Medical OOP Single", "OOP Single"], health_only=h)
    cm.oop_family        = p(["Current Health Plan OOP (family)", "Medical OOP Family", "OOP Family"], health_only=h)
    cm.ded_single        = p(["Current Health Plan Deductible (single)", "Medical Deductible Single", "Deductible Single"], health_only=h)
    cm.ded_family        = p(["Current Health Plan Deductible (family)", "Medical Deductible Family", "Deductible Family"], health_only=h)
    cm.er_cost           = p(["Current Health Plan ER Cost", "Medical ER Cost", "Employer Cost", "Employer Contribution"], health_only=h)
    cm.ee_cost           = p(["Current Health Plan EE Cost", "Medical EE Cost", "Employee Cost", "Employee Contribution"], health_only=h)
    cm.annual_salary     = p(["ANNUAL SALARY", "Annual Salary", "Salary", "Base Salary"])
    cm.hourly_rate       = p(["Hourly Rate", "Hourly Wage"])
    cm.hours_per_week    = p(["Hours Per Week", "Weekly Hours"])
    cm.notes             = p(["Notes", "Comments", "Remarks"])
    return cm


def find_dependent_slots(df: pd.DataFrame) -> tuple[list[DepSlot], bool]:
    """Return (slots, is_rowwise). Row-wise = one Relationship + one DOB col, repeated employee rows."""
    ...
    if row_rel or row_dob:
        return [DepSlot("rowwise", relationship_col=row_rel, dob_col=row_dob,
                        first_col=row_first, last_col=row_last, number=1)], True
    ...
    for h in df.columns:
        ...
        is_dob  = any(x in hn for x in ("dob", "date of birth", "birth date", "birthdate"))
        is_first = "first" in hn and "name" in hn
        is_last  = "last" in hn and "name" in hn
        is_rel   = any(x in hn for x in ("relationship", "relation", "type"))
        ...
        elif "child" in hn or "dependent" in hn or re.search(r"\bdep\b", hn):
            m = re.search(r"(\d+)", hn)
    ...
    complete = [s for s in slots.values() if s.dob_col or s.relationship_col or s.first_col or s.last_col]
    complete.sort(key=lambda s: (0 if s.default_relationship == "Spouse" else 1, s.number, s.slot))
...
def detect_horizontal_census(df: pd.DataFrame) -> tuple[bool, str]:
    ...
    has_dep = any(
        "dependent" in h or re.search(r"\bdep\b", h) or "spouse" in h or "child" in h
        for h in headers_norm
    )
    has_dep_health = any(
        ("dependent" in h or "spouse" in h or "child" in h)
        and ("dob" in h or "birth" in h or "relationship" in h)
        for h in headers_norm
    )
    if has_dep_health:
...
def _derive_ichra_class(row: pd.Series, cm: HorizontalColumnMap) -> str:
    ...
    ny_vt = state in {"NY", "VT"}
    if not worker_category:
        return "NY Full Time Benefits Eligible" if ny_vt else "Full Time Benefits Eligible"
    wc = _norm(worker_category)
    if "intern" in wc or "contractor" in wc:
        return ""
    if "part" in wc:
        return "NY Part Time Benefits Eligible" if ny_vt else "Part Time Benefits Eligible"
    return "NY Full Time Benefits Eligible" if ny_vt else "Full Time Benefits Eligible"
...
def _employee_group_key(row: pd.Series, cm: HorizontalColumnMap, row_index: int) -> tuple[Any, ...]:
    ...
    stable = (
        _cell(row, cm.employee_dob), _cell(row, cm.address1), _cell(row, cm.city),
        _cell(row, cm.state), _cell(row, cm.zip_code), _cell(row, cm.worksite_zip),
        _cell(row, cm.worker_category), _cell(row, cm.position_status),
        _cell(row, cm.annual_salary), _cell(row, cm.plan_name),
        _cell(row, cm.plan_tier), _cell(row, cm.health_waive_reason),
    )
    if any(stable):
        return ("anonymous", *stable)
...
def convert_horizontal_to_vertical(df: pd.DataFrame, options: HorizontalOptions) -> dict[str, Any]:
    ...
    if not dep_slots:
        notes.append({
            "Kind": "Warning",
            "Issue": "Horizontal mode was selected, but no dependent DOB/relationship columns were detected.",
        })
    ...
    for idx, row in df.iterrows():
        ...
        if _should_exclude_employee(row, cm, options):
            notes.append({"Kind": "Warning", "Issue": f"Input row {idx + 2} skipped (intern/contractor Worker Category)."})
            continue
    ...
    dep_num_col = _pick_col(df, ["Dependent Number", "Dependent #", "Dep Number", "Dep #"])
    ...
    for _, idxs in groups:
        ...
        if rowwise:
            ...
            for idx in idxs:
                ...
                dep_num = _cell(src, dep_num_col) or str(len(dep_records) + 1)
                if not any([rel_raw, dob, first, last]):
                    continue

                def _split(val: str) -> list[str]:
                    return [x.strip() for x in re.split(r"\s*[|;/]\s*", val) if x.strip()] or [val]

                rel_parts   = _split(rel_raw)
                dob_parts   = _split(dob)
                first_parts = _split(first)
                last_parts  = _split(last)
                part_count  = max(len(rel_parts), len(dob_parts), len(first_parts), len(last_parts), 1)
                if part_count > 1:
                    notes.append({"Kind": "Auto-fix", "Issue": f"Input row {idx + 2}: split one dependent cell into {part_count} rows."})

                for pi in range(part_count):
                    def _pick_part(parts: list[str], i: int) -> str:
                        return parts[i] if i < len(parts) else (parts[-1] if parts else "")

                    rel, note = normalize_relationship(_pick_part(rel_parts, pi))
                    if note:
                        notes.append({"Kind": "Auto-fix", "Issue": f"Input row {idx + 2}: {note}"})
                    dep_records.append({
                        "Relationship": rel,
                        "DOB":          _pick_part(dob_parts, pi),
                        "First Name":   _pick_part(first_parts, pi),
                        "Last Name":    _pick_part(last_parts, pi),
                        "_dep_number":  f"{dep_num}.{pi + 1}" if part_count > 1 else dep_num,
            ...
            for slot in dep_slots:
                ...
                if not any([explicit_rel_raw, dob, first, last]):
                    continue
                rel, note = normalize_relationship(explicit_rel_raw or slot.default_relationship)
                if note:
        ...
        # Resolve employee-level tier and election
        ...
        if not employee_election:
            if tier == "Waive" or raw_waive_reason:
                ...
                notes.append({
                    "Kind": "Warning",
                    "Issue": f"Input row {idxs[0] + 2}: no health election or medical plan found; defaulted employee to Enroll.",
                })
        ...
        if not tier:
            ...
            if employee_election == "Waive":
                ...
                notes.append({"Kind": "Warning", "Issue": f"Input row {src_row}: no health tier found; inferred '{tier}' from listed dependents."})
            elif raw_plan_name:
                tier = "Employee Only"
                notes.append({"Kind": "Warning", "Issue": f"Input row {src_row}: no health tier found; treated as Employee Only."})
            else:
                tier = "Waive"

        # Build employee output row
        employee = base_row()
        ...
        for dep in dep_records:
            ...
            dep_row["Health Election"]  = dependent_election_from_tier(
                employee_election, tier, dep_row["Relationship"]
            )
            output_rows.append(dep_row)

            src = dep.get("_source_row", "?")
            if not dep_row["Relationship"]:
                notes.append({"Kind": "Warning", "Issue": f"Input row {src}: dependent has no relationship."})
            if not dep_row["DOB"]:
                notes.append({"Kind": "Warning", "Issue": f"Input row {src}: dependent has no DOB."})
...
def validate_date(v: str) -> dict[str, Any]:
    ...
    if re.fullmatch(r"\d{5}", original):
        try:
            ...
            return {"ok": True, "msg": None, "fixed_val": fixed,
                    "fix_note": f"Excel date serial converted: '{original}' -> '{fixed}'"}
        except Exception:
            pass
...
def validate_cell(field_def: dict[str, Any], raw: str) -> dict[str, Any]:
    ...
    if ftype == "relationship":
        ...
        if canonical not in {"Employee", "Spouse", "Child"}:
            return {"ok": False,
                    ...
                    "fixed_val": canonical, "fix_note": note}
    ...
    if ftype == "tier":
        ...
        if canonical in _VALID_TIERS or canonical == "":
            return {"ok": True, "msg": None, "fixed_val": canonical, "fix_note": note}
        return {"ok": False,
                "msg": f"Invalid value '{v}' for {field_def['name']} — expected Employee Only, Employee + Spouse, Employee + Children, or Family",
                "fixed_val": v, "fix_note": None}

    if ftype == "zipcode":
        if v.upper() in _US_STATES:
            return {"ok": False,
                    "msg": f"{field_def['name']}: looks like a state abbreviation ('{v}'), not a zip code",
                    "fixed_val": v, "fix_note": None}
        zip5  = _pad_zip(v.split("-")[0])
        fixed = zip5 if len(zip5) == 5 else _pad_zip(v)
    ...
    if ftype == "numeric":
        ...
        if not re.fullmatch(r"-?\d+(\.\d+)?", clean):
            return {"ok": False,
                    "msg": f"{field_def['name']} must be numeric, got '{v}'",
                    "fixed_val": v, "fix_note": None}
    ...
    for df_ri, row in df.iterrows():
        ...
        for fi, field_def in enumerate(FIELDS):
            ...
            if fname in CLEARED_FIELDS:
                if raw:
                    clears.append({
                        "Row": display_row, "Col": field_def["col"],
                        "Field": fname, "Value": raw,
                        "Kind": "Cleared",
                        "Issue": f"Field '{fname}' automatically cleared",
                    })
                new_row[fname] = ""
                continue
            ...
            if result["fix_note"]:
                fixes.append({
                    "Row": display_row, "Col": field_def["col"],
                    "Field": fname, "Value": raw,
                    "Kind": "Auto-fix", "Issue": result["fix_note"],
                })
            new_row[fname] = result["fixed_val"] if result["ok"] else raw

        if zip_cache:
            for zip_fname, county_fname, col_letter in _ZIP_COUNTY_PAIRS:
                ...
                if info:
                    ...
                    warnings.append({
                        "Row": display_row, "Col": col_letter,
        ...
        pos = len(out_rows)
        (error_positions if row_has_error else valid_positions).append(pos)
        out_rows.append(new_row)

    # Cross-field: ER Cost / EE Cost pairing
...
def issues_to_csv_bytes(
    errors, warnings, fixes, clears, horizontal_notes=None
) -> bytes:
    ...
    if horizontal_notes:
        for n in horizontal_notes:
            combined.append({
                "Row": "—", "Col": "—", "Field": "Horizontal conversion",
                "Value": "", "Kind": n.get("Kind", "Note"), "Issue": n.get("Issue", ""),
            })
...
SAMPLE_VERTICAL_CSV = """\
First Name,Last Name,Relationship,DOB,Zip Code,Primary Worksite Zip Code,ICHRA Class,Health Election,Current Health Plan Tier,Annual Salary
...
SAMPLE_HORIZONTAL_CSV = """\
BIRTH DATE,PRIMARY ADDRESS LINE 1,PRIMARY ADDRESS - CITY,PRIMARY ADDRESS - STATE / TERRITORY,PRIMARY ADDRESS - ZIP CODE,WORKSITE ZIP CODE,WORKER CATEGORY,ANNUAL SALARY,MEDICAL PLAN NAME,MEDICAL COVERAGE TIER,Dependent Number,Dependent Relationship,Dependent DOB
...
_KIND_STYLES = {
    "Error":    "color:#993C1D;font-weight:600",
    "Warning":  "color:#854F0B;font-weight:600",
    "Auto-fix": "color:#3B6D11;font-weight:600",
    "Cleared":  "color:#555599;font-weight:600",
}


def _style_issues(issue_list: list[dict[str, Any]]) -> "pd.io.formats.style.Styler":
    cols = ["Row", "Col", "Field", "Value", "Kind", "Issue"]
    df   = pd.DataFrame(issue_list, columns=cols) if issue_list else pd.DataFrame(columns=cols)
    df   = df.sort_values(
        ["Row", "Kind"],
        key=lambda s: s.map(lambda x: str(x).zfill(6) if str(x).isdigit() else str(x)),
    ).reset_index(drop=True)
    styler = df.style
    color_fn = lambda val: _KIND_STYLES.get(val, "")
    return (
        styler.map(color_fn, subset=["Kind"])
        if hasattr(styler, "map")
        else styler.applymap(color_fn, subset=["Kind"])
    )


def _horizontal_notes_as_issues(horizontal_notes: list[dict[str, str]]) -> list[dict[str, Any]]:
    return [
        {
            "Row": "—", "Col": "—", "Field": "Horizontal conversion",
            "Value": "", "Kind": n.get("Kind", "Note"), "Issue": n.get("Issue", ""),
...
def main() -> None:
    st.set_page_config(page_title="Census Validator", page_icon=None, layout="wide")
    st.markdown(_CSS, unsafe_allow_html=True)

    st.title("Census Validator & Reformatter")
    st.caption(
        "Vertical CSA census validation  |  Horizontal dependent census conversion  |  "
        "Health-only plan/tier logic  |  System-ready CSV export"
    )
    ...
    with st.sidebar:
        ...
        infer_missing_tier = st.toggle(
            "Infer dependent enrollment when medical tier is missing", value=True,
            help="If the file has a medical plan but no tier, dependents are enrolled based on the dependent mix.",
        )
        inherit_worksite = st.toggle(
            "Copy worksite zip to dependent rows", value=True,
        ...
        for i, fdef in enumerate(FIELDS, 1):
            ...
            cleared = " [cleared]"   if fdef["name"] in CLEARED_FIELDS else ""
            st.caption(f"{i}. {fdef['name']}{req}{cleared}")
        st.caption("[required] = Required field   [cleared] = Always cleared in output")

    # ── Input area ───────────────────────────────────────────────────────────
    col_up, col_paste = st.columns(2, gap="large")
    with col_up:
        uploaded = st.file_uploader(
            "Upload census file (.csv / .xlsx / .xls)", type=["csv", "xlsx", "xls"]
        )
    with col_paste:
        pasted = st.text_area(
            "— or paste CSV data —",
    ...
    if run_btn:
        ...
        try:
            if uploaded:
                ...
                df_input = (
                    pd.read_csv(io.StringIO(pasted), dtype=str, keep_default_na=False)
                    .fillna("").astype(str)
                )
            else:
        ...
        if df_input is not None:
            with st.spinner("Validating and reformatting..."):
                ...
                use_horizontal = (
                    input_mode == "Horizontal dependent census"
                    or (horizontal_detected and input_mode != "Standard vertical census")
                )
    ...
    fixes    = res["fixes"]
    clears   = res["clears"]
    h_notes  = res.get("horizontal_notes", [])
    ...
    if res.get("horizontal_used"):
        ...
        st.markdown(
            ...
            f'{conv.get("source_rows", "?")} source rows '
            f'-> {conv.get("employee_groups", "?")} employee groups '
            f'-> {conv.get("output_rows", total)} output rows. '
            f'Dependents ordered: Employee, Spouse, Child.</div>',
    ...
    if res.get("mapping_notes") or res.get("unrecognised_cols"):
        with st.expander("Column mapping notes", expanded=bool(res.get("unrecognised_cols"))):
    ...
    # Metrics
    warn_count  = len(warnings) + sum(1 for n in h_notes if n.get("Kind") == "Warning")
    fix_count   = len(fixes)    + sum(1 for n in h_notes if n.get("Kind") == "Auto-fix")
    m1, m2, m3, m4, m5, m6, m7, m8 = st.columns(8)
    m1.metric("Total rows",    total)
    m2.metric("Valid rows",    total - err_rows)
    m3.metric("Error rows",    err_rows)
    m4.metric("Errors",        len(errors))
    m5.metric("Warnings",      warn_count)
    m6.metric("Auto-fixes",    fix_count)
    m7.metric("Cleared fields", len(clears))
    if res.get("horizontal_used") and "Relationship" in ref_df:
        m8.metric("Employees", int((ref_df["Relationship"] == "Employee").sum()))
    else:
        m8.metric("Mode", "Vertical")

    st.divider()
    st.subheader("Download")
    st.caption("Canonical 30-column order  |  Dates normalised  |  Health Election normalised  |  Dependents expanded (horizontal mode)")

    dc1, dc2, dc3, dc4 = st.columns(4)
    with dc1:
        st.markdown("**Full reformatted census**")
        st.caption(f"All {total} rows — ready for import")
        st.download_button("Download", data=df_to_csv_bytes(ref_df),
                           file_name="census_reformatted.csv", mime="text/csv",
                           use_container_width=True, key="dl_full")
    with dc2:
        st.markdown("**Valid rows only**")
        st.caption(f"{total - err_rows} rows — no validation errors")
        st.download_button("Download", data=df_to_csv_bytes(valid_df),
                           file_name="census_valid_rows.csv", mime="text/csv",
                           use_container_width=True, key="dl_valid")
    with dc3:
        st.markdown("**Error rows only**")
        st.caption(f"{err_rows} rows — needs review")
        st.download_button("Download", data=df_to_csv_bytes(error_df),
                           file_name="census_error_rows.csv", mime="text/csv",
                           use_container_width=True, key="dl_errors")
    with dc4:
        st.markdown("**Validation report**")
        st.caption("Errors, warnings, fixes, clears, horizontal notes")
        st.download_button("Download",
                           data=issues_to_csv_bytes(errors, warnings, fixes, clears, h_notes),
                           file_name="census_validation_report.csv", mime="text/csv",
                           use_container_width=True, key="dl_report")

    st.divider()

    h_as_issues = _horizontal_notes_as_issues(h_notes)
    all_issues  = errors + warnings + fixes + clears + h_as_issues
    ...
    if not all_issues:
        ...
        warn_list = warnings + [x for x in h_as_issues if x["Kind"] == "Warning"]
        fix_list  = fixes    + [x for x in h_as_issues if x["Kind"] == "Auto-fix"]

        tab_all, tab_err, tab_warn, tab_fix, tab_clr = st.tabs([
            f"All ({len(all_issues)})",
            f"Errors ({len(errors)})",
            f"Warnings ({len(warn_list)})",
            f"Auto-fixes ({len(fix_list)})",
        ...
        with tab_err:
            st.dataframe(_style_issues(errors), use_container_width=True, hide_index=True) \
                if errors else st.success("No errors found.")
        with tab_warn:
            st.dataframe(_style_issues(warn_list), use_container_width=True, hide_index=True) \
                if warn_list else st.success("No warnings found.")
        with tab_fix:
            st.dataframe(_style_issues(fix_list), use_container_width=True, hide_index=True) \
                if fix_list else st.info("No auto-fixes applied.")
        with tab_clr:
            st.dataframe(_style_issues(clears), use_container_width=True, hide_index=True) \
                if clears else st.info("No fields were cleared.")
